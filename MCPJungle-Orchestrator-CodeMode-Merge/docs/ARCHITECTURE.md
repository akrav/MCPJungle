...
## 12. Lambda Integration (Implemented)

> **Status:** ✅ Fully Implemented and Deployed (January 2026)
...
The Lambda integration is now complete, enabling serverless MCP tool execution on AWS Lambda.
...
│  │  2. If Lambda transport → use LambdaMcpClient                       │        │
│  │  3. Build URL: ?tool=context7&version=latest                        │        │
│  │  4. POST JSON-RPC to Lambda Function URL                            │        │
│  │                                                                     │        │
│  └───────────────────────────┬─────────────────────────────────────────┘        │
│                              │                                                  │
│                              ▼                                                  │
...
│  │                                                                     │        │
│  │  ┌─────────────────────────────────────────────────────────────┐    │        │
│  │  │  Dynamic Adapter (index.js)                                  │    │        │
│  │  │                                                              │    │        │
│  │  │  1. Parse tool & version from query params                   │    │        │
│  │  │  2. Check /tmp cache for package                             │    │        │
│  │  │  3. If cold: download from S3, unzip, cache                  │    │        │
│  │  │  4. Spawn MCP tool as child process                          │    │        │
│  │  │  5. Forward JSON-RPC, stream SSE response                    │    │        │
│  │  │                                                              │    │        │
│  │  └─────────────────────────────────────────────────────────────┘    │        │
│  └───────────────────────────┬─────────────────────────────────────────┘        │
│                              │                                                  │
...
|-----------|------|---------|
| Lambda Adapter | `orchestrator/infrastructure/lambda/src/index.js` | Serverless handler for MCP tools |
| Lambda Provisioner | `orchestrator/src/provisioning/lambda.ts` | Manages Lambda function lifecycle |
| Lambda MCP Client | `orchestrator/src/discovery/provisioning/lambdaClient.ts` | TypeScript client for Lambda calls |
| Go Lambda Transport | `internal/service/mcp/lambda.go` | Go client for Lambda-backed servers |
| Tool Packaging | `orchestrator/src/tools/packaging/` | NPM → S3 packaging pipeline |

### 12.3 Transport Types

The system now supports four MCP transport types:

| Transport | Use Case | Implementation |
|-----------|----------|----------------|
| `stdio` | Local processes | Spawn child process with stdin/stdout |
| `sse` | HTTP servers | Server-Sent Events over HTTP |
| `streamable_http` | Modern HTTP | Bidirectional HTTP streaming |
| `lambda` | **Serverless** | AWS Lambda with Function URL |

### 12.4 Usage Examples

**Register a Lambda-backed MCP server:**

```bash
# Using the CLI
mcpjungle register --name context7 \
  --transport lambda \
  --url "https://ovltvsbxtg6dznuuvwwtofc7sm0dspdg.lambda-url.us-east-1.on.aws" \
  --tool-name context7 \
  --tool-version latest
```
...
PROVISIONER=docker                   # none | docker | k8s | lambda
SUPABASE_URL=https://...             # For discovery
SUPABASE_KEY=...                     # Supabase anon key
OPENAI_API_KEY=sk-...                # For embeddings

# Lambda Configuration (when PROVISIONER=lambda)
AWS_REGION=us-east-1                 # AWS region
AWS_LAMBDA_FUNCTION_URL=https://...  # Lambda Function URL
AWS_LAMBDA_S3_BUCKET=mcp-artifacts   # S3 bucket for packages
AWS_LAMBDA_ROLE_ARN=arn:aws:iam::... # Lambda execution role
AWS_LAMBDA_ECR_REPO=...              # ECR repo for adapter image
LAMBDA_MEMORY_MB=1024                # Lambda memory (default: 1024)
LAMBDA_TIMEOUT_MS=30000              # Lambda timeout (default: 30s)
...
| **Lambda adapter** | `orchestrator/infrastructure/lambda/src/index.js` |
| **Lambda provisioner** | `orchestrator/src/provisioning/lambda.ts` |
| **Lambda MCP client** | `orchestrator/src/discovery/provisioning/lambdaClient.ts` |
| **Tool packaging** | `orchestrator/src/tools/packaging/` |
| **Lambda Terraform** | `orchestrator/infrastructure/lambda/main.tf` |
