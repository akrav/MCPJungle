...
export function loadConfig(
  ...
  const parsed = configSchema.safeParse({
    ...
    AWS_LAMBDA_FUNCTION_URL: env.AWS_LAMBDA_FUNCTION_URL,
    AWS_LAMBDA_S3_BUCKET: env.AWS_LAMBDA_S3_BUCKET,
    AWS_LAMBDA_ROLE_ARN: env.AWS_LAMBDA_ROLE_ARN,
    AWS_LAMBDA_ECR_REPO: env.AWS_LAMBDA_ECR_REPO,
    LAMBDA_MEMORY_MB: env.LAMBDA_MEMORY_MB,
    LAMBDA_TIMEOUT_MS: env.LAMBDA_TIMEOUT_MS,
    LAMBDA_EPHEMERAL_STORAGE_MB: env.LAMBDA_EPHEMERAL_STORAGE_MB,
  });
  if (!parsed.success) {
  ...
  return {
    ...
    awsLambdaFunctionUrl: data.AWS_LAMBDA_FUNCTION_URL,
    awsLambdaS3Bucket: data.AWS_LAMBDA_S3_BUCKET,
    awsLambdaRoleArn: data.AWS_LAMBDA_ROLE_ARN,
    awsLambdaEcrRepo: data.AWS_LAMBDA_ECR_REPO,
    lambdaMemoryMb: data.LAMBDA_MEMORY_MB,
    lambdaTimeoutMs: data.LAMBDA_TIMEOUT_MS,
    lambdaEphemeralStorageMb: data.LAMBDA_EPHEMERAL_STORAGE_MB,
  };
}
