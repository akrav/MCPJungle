...
const (
	...
	TransportLambda         McpServerTransport = "lambda"
)
...
type RegisterServerInput struct {
	...
	// ToolName is the name of the MCP tool to load.
	// It is mandatory when the transport is "lambda".
	ToolName string `json:"tool_name"`

	// ToolVersion is the version of the MCP tool to load.
	// Used when the transport is "lambda". Defaults to "latest".
...
func ValidateTransport(input string) (McpServerTransport, error) {
	errMsgExt := fmt.Sprintf(
		"(acceptable values: '%s', '%s', '%s', '%s')", TransportStreamableHTTP, TransportStdio, TransportSSE, TransportLambda,
	)
